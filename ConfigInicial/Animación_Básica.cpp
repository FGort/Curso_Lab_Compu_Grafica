...
//variables para animación de movimiento vertical de pelota 
bool AnimBall2 = false; //Animación de movimiento vertical de pelota
...
int main()
{
	...
	//models
	Model Dog((char*)"Models/Previo10/RedDog.obj");
	Model Piso((char*)"Models/Previo10/piso.obj");
	Model Ball((char*)"Models/Previo10/ball.obj");
...
void Animation() {
	...
	if (AnimBall2)
	{
		if (sube) //la pelota esta subiendo
		{
			posBall += 0.003f; //aumenta la altura de poco en poco
			if (posBall >= pointLightPositions[0].y) //si la pelota llega a la altura de la luz
			{
				sube = false; //la pelota empieza a bajar
			}
		}
		else //la pelota no esta subiendo, entonces esta bajando
		{
			posBall -= 0.003f; //disminuye la altura de poco en poco
			if (posBall <= 0.0f) //si la pelota llega al origen (nariz del perro)
			{
				sube = true; //la pelota empieza a subir
			}
		}
